...
CHANNEL_MEAN = [0.1739, 0.5513, 0.1149, 0.4873, 0.4495]
CHANNEL_STD  = [0.0768, 0.4126, 0.1277, 0.2191, 0.2256]
...
class MyDataset(Dataset):
    ...
    def __init__(
        ...
        graben_erosion: bool = False,
    ):
        self.images_dir = images_dir
        self.masks_dir = masks_dir
        self.graben_erosion = graben_erosion
        ...
        if valid_list_file is not None:
            ...
            all_files = [f for f in all_files if f in valid_set]
            print(f'[MyDataset] valid_list 过滤: {before} -> {len(all_files)} '
                  f'(剔除 {before - len(all_files)} 张, 列表={os.path.basename(valid_list_file)})')
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, str]:
        ...
        # ---- Graben 形态增强: 随机腐蚀, 宽谷→细线 ----
        if self.graben_erosion and np.random.random() < 0.5:
            graben = (mask == 4)
            if graben.any():
                from skimage.morphology import binary_erosion
                eroded = binary_erosion(graben)
                if eroded.any():
                    mask[graben] = 0
                    mask[eroded] = 4
